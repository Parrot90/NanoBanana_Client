...
export function WomenPromptCard({ image, prompt }: { image?: string; prompt: string }) {
  ...
  const generateWithPrompt = async () => {
    try {
      ...
      // copy failure should not block navigation
    }
    ...
    <article className="bg-white/95 rounded-2xl shadow-sm border border-black/10 overflow-hidden flex flex-col">
      <div className="p-3">
        <div className="aspect-[3/4] w-full overflow-hidden rounded-xl">
          <img
            src={image || "/placeholder.svg?height=600&width=480&query=woman%20portrait%20cinematic"}
